...
class MultiQueryAttention(nn.Module):
    def __init__(self, hidden_size, head_num):
        ...
        self.W_k = nn.Linear(hidden_size, self.head_size)
    ...
    def forward(self, x):
        ...
        K = self.W_k(x).unsqueeze(2).transpose(1, 2)
        V = self.W_v(x).unsqueeze(2).transpose(1, 2)
...
my_attention = MultiQueryAttention(hidden_size, num_heads)
